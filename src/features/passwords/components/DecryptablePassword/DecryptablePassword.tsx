		...
		if (password.plaintext === null) {
			...
			return (
				<div className='flex items-center'>
					{password.visible ? (
						<EyeSlashIcon
							className='h-7 w-7 pt-0.5 text-zinc-500 cursor-pointer'
					...
					{copied ? (
						...
						<ClipboardDocumentIcon
							onClick={onCopy}
							className='h-7 w-7 pt-0.5 ml-3 text-zinc-500 cursor-pointer'
						/>
					)}
				</div>
			);
	...
	return (
		<>
			<div className='DecryptablePassword w-full flex flex-col items-stretch justify-between mb-5 rounded-md'>
				<div className='min-h-[96px] w-full flex flex-col items-stretch justify-between p-4 bg-zinc-200 rounded-md'>
					...
					<div className='h-2/3 w-full flex items-start justify-between'>
						<input
							...
							className={`w-full pr-2 break-all rounded-md text-lg text-zinc-900 bg-transparent ${
