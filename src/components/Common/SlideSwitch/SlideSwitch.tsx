...
export function SlideSwitch(props: SlideSwitchProps) {
  ...
  const [active, setActive] = useState<string>(selected || leftValue);
  ...
  const getBtnTextColor = (v: string) => (active === v ? "text-slate-50" : "light:text-slate-900");
    ...
    <div className="SlideSwitch h-[56px]">
      ...
      >
        <button
          ...
          className={`z-10 w-1/2 px-4 py-2 rounded-l-full ${getBtnTextColor(
            leftValue,
        ...
        <button
          ...
          className={`z-10 w-1/2 px-4 py-2 rounded-r-full ${getBtnTextColor(
            rightValue,
          )} ring-inset ring-1 ring-slate-700`.trim()}
        >
          {rightValue}
